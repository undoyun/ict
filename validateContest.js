"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.handleValidation = exports.updateContestValidation = exports.createContestValidation = void 0;
var express_validator_1 = require("express-validator");
var Machine_1 = require("../models/Machine");
/**
 * Validation rules for creating a new contest.
 */
exports.createContestValidation = [
    (0, express_validator_1.body)('name')
        .notEmpty()
        .withMessage('Name is required.')
        .isLength({ min: 3 })
        .withMessage('Name must be at least 3 characters long.'),
    (0, express_validator_1.body)('description')
        .optional()
        .isString()
        .withMessage('Description must be a string.')
        .isLength({ max: 500 })
        .withMessage('Description can be at most 500 characters long.'),
    (0, express_validator_1.body)('startTime')
        .notEmpty()
        .withMessage('Start time is required.')
        .isISO8601()
        .withMessage('Start time must be a valid ISO 8601 date.')
        .toDate(),
    (0, express_validator_1.body)('endTime')
        .notEmpty()
        .withMessage('End time is required.')
        .isISO8601()
        .withMessage('End time must be a valid ISO 8601 date.')
        .toDate()
        .custom(function (value, _a) {
        var req = _a.req;
        var startTime = new Date(req.body.startTime);
        var endTime = new Date(value);
        if (endTime <= startTime) {
            throw new Error('End time must be after start time.');
        }
        var durationInHours = (endTime.getTime() - startTime.getTime()) / (1000 * 60 * 60);
        if (durationInHours < 24) {
            throw new Error('Contest duration must be at least 24 hours.');
        }
        return true;
    }),
    (0, express_validator_1.body)('machines')
        .isArray({ min: 1 })
        .withMessage('Machines must be an array with at least one machine ID.')
        .custom(function (machines) { return __awaiter(void 0, void 0, void 0, function () {
        var machineCount;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, Machine_1.default.countDocuments({ _id: { $in: machines } })];
                case 1:
                    machineCount = _a.sent();
                    if (machineCount !== machines.length) {
                        throw new Error('One or more machines are invalid.');
                    }
                    return [2 /*return*/, true];
            }
        });
    }); }),
    (0, express_validator_1.body)('contestExp')
        .notEmpty()
        .withMessage('Contest EXP is required.')
        .isInt({ min: 0 })
        .withMessage('Contest EXP must be a non-negative integer.')
];
/**
 * Validation rules for updating an existing contest.
 */
exports.updateContestValidation = [
    (0, express_validator_1.body)('name')
        .optional()
        .isLength({ min: 3 })
        .withMessage('Name must be at least 3 characters long.'),
    (0, express_validator_1.body)('description')
        .optional()
        .isString()
        .withMessage('Description must be a string.')
        .isLength({ max: 500 })
        .withMessage('Description can be at most 500 characters long.'),
    (0, express_validator_1.body)('startTime')
        .optional()
        .isISO8601()
        .withMessage('Start time must be a valid ISO 8601 date.')
        .toDate(),
    (0, express_validator_1.body)('endTime')
        .optional()
        .isISO8601()
        .withMessage('End time must be a valid ISO 8601 date.')
        .toDate()
        .custom(function (value, _a) {
        var req = _a.req;
        var startTime = req.body.startTime
            ? new Date(req.body.startTime)
            : (req.body.existingStartTime ? new Date(req.body.existingStartTime) : null);
        if (startTime && value) {
            if (new Date(value) <= startTime) {
                throw new Error('End time must be after start time.');
            }
            var durationInHours = (new Date(value).getTime() - startTime.getTime()) / (1000 * 60 * 60);
            if (durationInHours < 24) {
                throw new Error('Contest duration must be at least 24 hours.');
            }
        }
        return true;
    }),
    (0, express_validator_1.body)('machines')
        .optional()
        .isArray({ min: 1 })
        .withMessage('Machines must be an array with at least one machine ID.')
        .custom(function (machines) { return __awaiter(void 0, void 0, void 0, function () {
        var machineCount;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, Machine_1.default.countDocuments({ _id: { $in: machines } })];
                case 1:
                    machineCount = _a.sent();
                    if (machineCount !== machines.length) {
                        throw new Error('One or more machines are invalid.');
                    }
                    return [2 /*return*/, true];
            }
        });
    }); }),
    (0, express_validator_1.body)('contestExp')
        .optional()
        .isInt({ min: 0 })
        .withMessage('Contest EXP must be a non-negative integer.')
];
/**
 * Middleware to handle validation results.
 */
var handleValidation = function (req, res, next) {
    var errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        res.status(400).json({ errors: errors.array() });
        return;
    }
    next();
};
exports.handleValidation = handleValidation;
